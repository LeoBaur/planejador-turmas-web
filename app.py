...
# =========================
# INTERFACE E PROCESSAMENTO (TOP-OFF)
# =========================
    ...
    if st.button("🚀 Processar e Salvar", type="primary"):
        try:
            ...
            for curso in df_motor["Curso"].unique():
                ...
                # Preenchimento de turmas existentes
...
if not df_final_trabalho.empty:
    ...
    # Salvamento Automático por Threading
    ...
    with col_r:
        ...
        if not baixas.empty:
            st.warning(f"⚠️ {len(baixas)} turmas abaixo do mínimo")
    ...
    # =========================
    # RELATÓRIO: AGUARDANDO ATENDIMENTO (AJUSTADO)
    # =========================
    st.divider()
    ...
    for _, row in df_final_trabalho.iterrows():
        ...
        if qtd_aguardando > 0:
            ...
            for c, q in cnpjs_row.items():
                ...
                if qtd_pendente > 0:
                    pendencias.append({"UF": uf_linha, "CNPJ": c, "Qtd Aguardando": qtd_pendente})
    ...
    if pendencias:
        ...
        df_total_uf = df_total_uf.rename(columns={"Qtd Aguardando": "Total de Vagas Aguardando"})

        st.write("**Total de Vagas Aguardando Atendimento por UF:**")
        st.table(df_total_uf)
        
        st.write("**Detalhamento por CNPJ:**")
        ...
        st.download_button(
            label="📥 Baixar Relatório (Excel com Totais por UF)",
